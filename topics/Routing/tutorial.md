...
Make sure `index.js` or `main.jsx` (depending on the bundler you used) is actually boring:
...
```diff
// tsx lines=[2,7-9] filename=src/app.jsx
...
```diff
// tsx lines=[2,4-5,8-9,13-19] filename=src/app.jsx
...
```diff
// jsx lines=[15-18] filename=src/app.jsx
...
However, before it will work we need to render an `Outlet` in the `App.jsx` "parent" route.
...
```diff
// js lines=[5-12] filename=src/app.jsx
...
```diff
// js lines=[4-6] filename=src/app.jsx
...
```diff
// jsx filename=src/app.jsx lines=[5-12]
